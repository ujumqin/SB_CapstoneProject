...
* Overwatch online communities contain a fair amount of negativity
* This negativity may spill over in game via the chat
* A prototype to automatically detect negative language was developed
...
## Conclusion
From our analysis, we can see that the Overwatch community is experiencing some unrest. With about 40% of comments containing negative sentiment, we should be concerned. From the word clouds, it seemed the hero names were appearing prominently so there may be a correlation to them and the negativity. The silver lining in finding the high amount of negativity is we can harness this data to help us create an algorithm to address negative language automatically. From this exploratory analysis we are able to create models that will capture negative sentiment automatically. Our best model had an accuracy of 83%. When looking at the ROC curve, we can see that at 83% we will experience approximately a 2% false positive rate. Ideally, we reduce this false positive rate as much as possible since we do not want to punish too many players for not doing wrong. With this, the 83% detection rate with a 2% false positive is healthy and should be used. 
![ROC Curve](ROC_bestmodel.png)

## Next Steps
Further research regarding the negative sentiment should be performed. It may yield some interesting thoughts on changes that have occurred in the game. Additionally, tracking this data over time may also help with understanding the community better and may be worth investigating. In regards to the algorithm, it would be worth improving by using actual customer complaints. Additionally we should look into building our own lexicon for Overwatch players. With these two pieces combined, we can better create an algorithm that will free up our customer service representatives to do more valuable work, such as restoring accounts or addressing billing issues, instead of having to deal with bad language reports. For now this algorithm works as a prototype to show how this tool can be effective in addressing this complaints. Doing all of these will help us address this negativity much more efficiently and quickly which can help improve the community for Overwatch players. This in turn can help improve the reputation of the game overall. 

\newpage

# Appendix
The appendix contains the code used for this project. To obtain the files and the code below, visit: https://github.com/ujumqin/SB_CapstoneProject 
